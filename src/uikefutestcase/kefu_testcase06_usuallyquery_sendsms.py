...
class KefuTestcase06UsuallyquerySendSms(unittest.TestCase):
    def setUp(self):
    ...
    def test_kefu_testcase06_usuallyquery_sendsms(self):
        driver = self.driver
        ...
        self.assertEqual(driver.title,u"客服系统")
        driver.find_element_by_css_selector("div.container>div.navbar-collapse.collapse.navbar-responsive-collapse>ul.nav.navbar-nav>li:nth-child("+appobjectkefu.kefu_tab_usuallyquery+").dropdown>ul.dropdown-menu>li:nth-child(8)>a").click()
        self.assertEqual(driver.title,u"客服系统")
        ...
        ''''
        print " the kefusendsms is ",kefusendsms
        #if send sms is ok, the statu need be 200
        if u"发送成功" in kefusendsms:
